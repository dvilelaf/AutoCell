    ...
    def step(self):
        ...
        for cellIndex in cellIndexList:
            ...
            if cell.isAlive():
                ...
                action, positionDelta = cell.selectAction(environment)
                position = ((cell.row + positionDelta[0]) % self.height, (cell.col + positionDelta[1]) % self.width)
                target = self.get(position[0], position[1])
